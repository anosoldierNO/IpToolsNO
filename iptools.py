...
print("Sjekker DNS lekkasje, vennligst vent.....")

#DNSLEAKTEST 
#Mostly written by AnoSoldierNO
##########################################
def ping(host):
...
with MoonSpinner('Henter informasjon…') as bar:
    for i in range(100):
        sleep(0.02)
        bar.next()
#Usikker på om jeg trenger å kjære next kommando her som på linjen over??
data = response.read().decode("utf-8")
parsed_data = json.loads(data)
print()
print('\033[36;5;236mDin IP informasjon: \033[0;0m')
...
if servers == 0:
    print("Ingen DNS server funnet")

else:
    print("Du bruker " + str(servers) + " DNS servere:")
    for dns_server in parsed_data:
        if dns_server['type'] == "dns":
            if dns_server['country_name']:
                if dns_server['asn']:
...
#Most cut and pasted from OpenSource API on Speedtest, but did some adjustments
